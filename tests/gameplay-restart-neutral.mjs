...
closeTo(run('state.beacons[0].energy'), 31.9);
...
console.log('gameplay restart neutral passed: held movement is suppressed without freezing the run, keyboard repeat stays one retry edge, Start retry is atomic, and reconnect-held does not create a duplicate Start edge');
